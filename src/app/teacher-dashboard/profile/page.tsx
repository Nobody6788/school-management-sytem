...
// Mocking the logged-in teacher. In a real app, this would come from an auth context.
const loggedInTeacher = initialTeachers.find(t => t.id === 'T02');

export default function TeacherProfilePage() {
  const [teacherProfile, setTeacherProfile] = useState({ ...loggedInTeacher });
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onload = (loadEvent) => {
        if (loadEvent.target && typeof loadEvent.target.result === 'string') {
          setTeacherProfile((prev) => ({ ...prev!, profilePicture: loadEvent.target.result as string }));
        }
      };
  ...
  const handleSave = () => {
    ...
    const originalTeacherIndex = initialTeachers.findIndex(t => t.id === teacherProfile.id);
    if (originalTeacherIndex > -1) {
        initialTeachers[originalTeacherIndex] = { ...teacherProfile };
    }
  ...
  const handleCancel = () => {
    setTeacherProfile({ ...loggedInTeacher });
    setIsEditing(false);
  }
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
                <Avatar className="h-20 w-20 border">
                    ...
                    />
                    <AvatarFallback>{teacherProfile.name.slice(0, 2).toUpperCase()}</AvatarFallback>
                </Avatar>
                {isEditing && (
                    <Button 
                        size="icon" 
