...
export default function ParentMessagingPage() {
    ...
    const getParticipantInfo = (type: string, id: string): { name: string; email?: string } => {
        switch (type) {
            ...
            case 'Parent': 
                ...
                return { name: parent?.name || 'Unknown Parent', email: parent?.email };
    ...
    const recipientOptions: ComboboxOption[] = [
        ...
        {
            value: `Admin:${adminProfile.id}`,
            label: `${adminProfile.name} (Admin)`,
        }
    ].sort((a, b) => (a.label as string).localeCompare(b.label as string));

    const inboxMessages = messages
        .filter(m => m.recipientId === loggedInParent.id && m.recipientType === 'Parent')
        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        
    const sentMessages = messages
        .filter(m => m.senderId === loggedInParent.id && m.senderType === 'Parent')
        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        ...
        if (recipientInfo.email) {
            try {
                await sendEmail({
                    ...
                    body: `You have received a new message from ${loggedInParent.name}.\n\n---\n\n${body}\n\n---\n\nPlease log in to CampusFlow to view and reply.`
                });
                ...
                <CardHeader>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <CardDescription>Communicate with teachers and administration.</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                <CardContent>
                                ...
                                <div className="grid grid-cols-6 items-center gap-4">
                                    ...
                                    <div className="col-span-5">
                                        <Combobox
                                            ...
                                            placeholder="Search for a teacher or admin..."
